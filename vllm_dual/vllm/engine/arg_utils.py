...
from vllm.config import (BlockSize, CacheConfig, CacheDType, CompilationConfig,
                         ...
                         DualModelConfig, ArbiterModelConfig, 
...
class EngineArgs:
    ...
    arbiter_model_config: Optional[Dict[str, Any]] = None
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument(
            ...
            type=json.loads,
            default=None,
            help='Dual model configuration in JSON format.'
        )

        parser.add_argument(
            '--arbiter-model-config',
            type=json.loads,
            default=EngineArgs.arbiter_model_config,
            help='Arbiter model configuration in JSON format.')
    ...
    def create_engine_config(
        ...
        if self.dual_model_config is not None:
            if ("small_model_engine_args" in self.dual_model_config
                    and isinstance(self.dual_model_config[
                        "small_model_engine_args"], dict)):
                self.dual_model_config["small_model_engine_args"] = (
                    EngineArgs(**self.dual_model_config[
                        "small_model_engine_args"]))
            dual_model_config = DualModelConfig(large_worker_cls=parallel_config.worker_cls, large_worker_tensor_parallel_size=parallel_config.tensor_parallel_size, **self.dual_model_config)
            parallel_config.worker_cls = "vllm.worker.dual_worker.DualModelWorker"

        arbiter_model_config = None
        if self.arbiter_model_config is not None:
            # Convert list to set for trigger_token_ids
            if "trigger_token_ids" in self.arbiter_model_config and isinstance(self.arbiter_model_config["trigger_token_ids"], list):
                self.arbiter_model_config["trigger_token_ids"] = set(self.arbiter_model_config["trigger_token_ids"])
            
            # Convert dict to EngineArgs for nested configs
            for key in ["controller_engine_args", "prefix_engine_args"]:
                if key in self.arbiter_model_config and isinstance(self.arbiter_model_config[key], dict):
                    # We use EngineArgs here, make sure imports are fine or use type(self)
                    self.arbiter_model_config[key] = EngineArgs(**self.arbiter_model_config[key])

            arbiter_model_config = ArbiterModelConfig(**self.arbiter_model_config)
            parallel_config.worker_cls = arbiter_model_config.worker_cls or "vllm.worker.arbiter_worker.HiddenStateArbiterWorker"
        ...
        config = VllmConfig(
            ...
            arbiter_model_config=arbiter_model_config,
