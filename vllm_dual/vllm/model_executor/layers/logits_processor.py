...
print('VLLM_LOGITS_PROCESSOR_THREADS=', envs.VLLM_LOGITS_PROCESSOR_THREADS)
    ...
    def forward(
        ...
        if logits is not None:
            ...
            if sampling_metadata is not None and \
                ...
                _apply_logits_processors_fn = _apply_logits_processors
                logits_processors = sampling_metadata.seq_groups[0].sampling_params.logits_processors
                use_parallel = logits_processors and any(getattr(x, 'use_parallel', None) for x in logits_processors)
                if use_parallel:
                    _apply_logits_processors_fn = _apply_logits_processors_multi_seq
                logits = _apply_logits_processors_fn(logits, sampling_metadata)
                end_time = time.perf_counter()
                self.timer += end_time - start_time
                self.cnt += logits.shape[0]
                # if self.cnt % 1000 == 0:
                #     print('logits_processor time:', self.timer / self.cnt)
...
def _apply_logits_processors_multi_seq(
    ...
    # logits_row_ids_and_logits_row_futures = []
    logits_processors = sampling_metadata.seq_groups[0].sampling_params.logits_processors
    assert all(seq_group.sampling_params.logits_processors == logits_processors for seq_group in sampling_metadata.seq_groups)
