...
class CompletionOutput:
    ...
    is_intervened: Optional[list[int]] = None
    arbiter_prediction: Optional[list[int]] = None
...
class RequestOutput:
    ...
    def from_seq_group(
        ...
        for i, seq in enumerate(top_n_seqs):
            ...
            output_token_ids, is_intervened, arbiter_prediction = seq.get_output_token_ids_to_return(delta)
            ...
            if use_cache:
                ...
                if isinstance(output_token_ids, int):
                    assert isinstance(is_intervened, bool)
                    output.token_ids.clear()
                    output.token_ids.append(output_token_ids)
                    output.is_intervened.append(is_intervened)
                    output.arbiter_prediction.append(arbiter_prediction)
                else:
                    assert len(output_token_ids) == len(is_intervened), pdb.set_trace()
                    output.token_ids = output_token_ids
                    output.is_intervened = is_intervened
                    output.arbiter_prediction = arbiter_prediction
                ...
                output_token_ids = [output_token_ids] if isinstance(output_token_ids, int) else output_token_ids
                is_intervened = [is_intervened] if isinstance(is_intervened, bool) else is_intervened
                arbiter_prediction = [arbiter_prediction] if isinstance(arbiter_prediction, int) else arbiter_prediction
                assert len(output_token_ids) == len(is_intervened), pdb.set_trace()
                output = CompletionOutput(
                    ...
                    arbiter_prediction=arbiter_prediction)
