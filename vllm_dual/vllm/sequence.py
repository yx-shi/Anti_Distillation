...
class SequenceData(msgspec.Struct,
    ...
    _is_intervened: Optional[array] = msgspec.field(default_factory=list)
    _arbiter_prediction: Optional[array] = msgspec.field(
        default_factory=lambda: array(VLLM_TOKEN_ID_ARRAY_TYPE, []))
    ...
    @property
    def is_intervened(self) -> tuple[int, ...]:
        return tuple(self._is_intervened)

    @is_intervened.setter
    def is_intervened(self,
                        new_is_intervened: GenericSequence[bool]) -> None:
        self._is_intervened = array('b', new_is_intervened)

    @property
    def arbiter_prediction(self) -> tuple[int, ...]:
        return tuple(self._arbiter_prediction)

    @arbiter_prediction.setter
    def arbiter_prediction(self,
                        new_arbiter_prediction: GenericSequence[int]) -> None:
        self._arbiter_prediction = array(VLLM_TOKEN_ID_ARRAY_TYPE, new_arbiter_prediction)
    ...
    def append_token_id(self, token_id: int, logprob: float, is_intervened: Optional[bool] = False, arbiter_prediction: Optional[int] = -1) -> None:
        self._output_token_ids.append(token_id)
        self._new_appended_tokens.append(token_id)
        self._cached_all_token_ids.append(token_id)
        self._cumulative_logprob += logprob
        self._is_intervened.append(is_intervened)
        self._arbiter_prediction.append(arbiter_prediction)
    ...
    def get_is_intervened(self) -> tuple[int, ...]:
        return self.is_intervened
    ...
    def apply_delta(self, delta: SequenceDataDelta):
        raise NotImplementedError("is_intervened not processed for use_delta scenario")
    ...
    def __repr__(self) -> str:
        return (f"SequenceData("
                ...
                f"is_intervened={self.is_intervened}, "
...
class Sequence:
    ...
    def get_output_token_ids_to_return(
            self, delta: bool) -> Tuple[Union[GenericSequence[int], int], Union[GenericSequence[bool], bool], Union[GenericSequence[int], int]]:
        """If delta is True, only new tokens since the last call to
        this method are returned"""
        if not delta:
            return self.get_output_token_ids(), self.get_is_intervened(), self.get_arbiter_prediction()
        ...
        if num_new_tokens == 1:
            ...
            return self.data._cached_all_token_ids[-1], self.data.is_intervened[-1], self.data.arbiter_prediction[-1]

        if num_new_tokens == 0:
            return [], [], []

        return self.data._cached_all_token_ids[-num_new_tokens:], self.data.is_intervened[-num_new_tokens:], self.data.arbiter_prediction[-num_new_tokens:]
    ...
    def append_token_id(self, token_id: int, logprobs: dict[int, Logprob], is_intervened: Optional[bool] = False, arbiter_prediction: Optional[int] = -1) -> None:
        assert token_id in logprobs
        self.output_logprobs.append(logprobs)
        self.data.append_token_id(token_id, logprobs[token_id].logprob, is_intervened=is_intervened, arbiter_prediction=arbiter_prediction)
    ...
    def get_is_intervened(self) -> Optional[list[bool]]:
        return self.data.get_is_intervened()

    def get_arbiter_prediction(self) -> tuple[int, ...]:
        return self.data.arbiter_prediction
...
class SequenceOutput(
    ...
    is_intervened: Optional[bool] = False
    arbiter_prediction: Optional[int] = -1
    ...
    def __repr__(self) -> str:
        return (f"SequenceOutput(parent_seq_id={self.parent_seq_id}, "
                ...
                f"is_intervened={self.is_intervened}), "
                f"arbiter_prediction={self.arbiter_prediction})")
