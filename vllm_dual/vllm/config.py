...
from typing import (TYPE_CHECKING, Any, Callable, ClassVar, Final, Literal, Dict, 
                    Optional, Protocol, TypeVar, Union, get_args, Set, List)
...
class DualModelConfig:
    ...
    large_worker_cls: str
    large_worker_tensor_parallel_size: int
    special_token_id: int = -1
    adversarial_mode: str = "special_token"
    hard_candidate_top_k: int = 20
    hard_candidate_top_p: float = 1.0
    soft_student_weight: float = 1.0
    soft_temperature: float = 1.0
    debug_log_interval: int = 16

@dataclass
class ArbiterModelConfig:
    """
    放到 vllm_config.arbiter_model_config，结构参考 dual_worker 的 dual_model_config。
    """
    controller_engine_args: Optional["EngineArgs"] = None
    prefix_engine_args: Optional["EngineArgs"] = None
    arbiter_input_dim: Optional[int] = None

    gate_temperature: float = 1.0

    arbiter_state_dict: Optional[Dict[str, torch.Tensor]] = None
    arbiter_ckpt_path: Optional[str] = None
    arbiter_state_dict_prefix: str = ""

    # For KeywordArbiterWorker
    trigger_token_ids: Optional[Set[int]] = None
    intervention_candidates: Optional[List[List[int]]] = None
...
class VllmConfig:
    ...
    arbiter_model_config: Optional[ArbiterModelConfig] = None
