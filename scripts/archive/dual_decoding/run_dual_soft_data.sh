...
# vLLM-dual token-level 正式数据构建：只跑 teacher_token_soft。
...
CANDIDATE_DIR="${RESULT_ROOT}/candidates/${EXPERIMENT_NAME}/soft_bs96_s80"
SHARD_DIR="${CANDIDATE_DIR}/shards"
ANALYSIS_DIR="${RESULT_ROOT}/analysis/${EXPERIMENT_NAME}/soft_bs96_s80"
...
SOFT_STUDENT_WEIGHT="${SOFT_STUDENT_WEIGHT:-0.5}"
...
run_soft_generate() {
  local pids=()
  local shard_logs=()
  local aggregate_log="${LOG_DIR}/teacher_generate_teacher_token_soft.log"
  guard_output_file "$(candidate_file)"
  guard_output_file "${aggregate_log}"

  echo
  echo "[$(date '+%F %T')] launching teacher_token_soft generation: shards=${GENERATION_NUM_SHARDS}, TP=${TEACHER_TP_SIZE}, GPUs=${GENERATION_GPU_IDS}, prompt_batch=${GEN_PROMPT_BATCH_SIZE}, max_num_seqs=${GEN_MAX_NUM_SEQS}"
  for ((shard_idx = 0; shard_idx < GENERATION_NUM_SHARDS; shard_idx++)); do
    local gpu_id="${GENERATION_GPU_ID_LIST[$shard_idx]}"
    local shard_file
    local shard_log
    shard_file="$(candidate_shard_file "${shard_idx}")"
    shard_log="${LOG_DIR}/teacher_generate_teacher_token_soft_shard_$(printf "%02d" "${shard_idx}").log"
    guard_output_file "${shard_file}"
    guard_output_file "${shard_log}"
    shard_logs+=("${shard_log}")

    echo "[$(date '+%F %T')] teacher_token_soft shard ${shard_idx}/${GENERATION_NUM_SHARDS} -> GPU ${gpu_id}"
    (
      CUDA_VISIBLE_DEVICES="${gpu_id}" \
      ${CONDA_RUN} python src/vllm_dual_decoding/teacher_generate.py \
        --generation-mode soft \
  ...
  if ! wait_for_pids "${pids[@]}"; then
    echo "At least one teacher_token_soft generation shard failed. Logs:" >&2
    printf '  %s\n' "${shard_logs[@]}" >&2
    exit 1
  fi
  ...
  merge_candidate_shards
  {
    echo "teacher_token_soft_generate_done shards=${GENERATION_NUM_SHARDS} output_file=$(candidate_file)"
...
echo
echo "teacher_token_soft data build finished."
