...
token-level 新主链路通过 `src/experiment/dataset_registry.py` 注册数据集：

- `deepscaler`：`agentica-org/DeepScaleR-Preview-Dataset/default`，train split 生成/训练，train split holdout eval，字段为 `problem` / `answer` / `solution`。
- `gsm8k`：`openai/gsm8k/main`，train split 生成/训练，test split eval，字段为 `question` / `answer` / `answer`。
...
- GSM8K token-level 配置可通过 `--allow-hash-answer-fallback` 打开 `#### ...` 模型输出兜底；该开关默认关闭，因此 DeepScaleR 评分语义保持 boxed / `Final Answer:`。
...
```bash
conda run --no-capture-output -n adistill-unified python src/run_experiment.py \
  --config configs/deepscaler.yaml \
...
data-side 阶段不要求产出 checkpoint 或 eval JSON；这些属于 full smoke。SFT-ready distill JSONL 由 `src/run_experiment.py --stage build_distill` 构建，内部复用 `src/vllm_dual_decoding/build_distill_dataset.py`。

新配置驱动链路使用统一 run_id：

```text
{experiment_group}__ds-{dataset}__mode-{mode}__k-{hard_candidate_top_k|na}__w-{soft_student_weight|na}__t-{temperature|na}__p-{top_p|na}__n-{max_samples|na}__seed-{subset_seed|na}
```

新输出布局：

```text
result/vllm_dual_decoding/candidates/{run_id}/candidate_pool.jsonl
result/vllm_dual_decoding/candidates/{run_id}/scored_candidates.jsonl
result/vllm_dual_decoding/datasets/{run_id}/distill.jsonl
result/vllm_dual_decoding/datasets/{run_id}/eval.jsonl
result/vllm_dual_decoding/analysis/{run_id}/final_eval.json
/home/disk2/shiyixuan/Anti_Distillation/result/vllm_dual_decoding/runs/{run_id}/
```
