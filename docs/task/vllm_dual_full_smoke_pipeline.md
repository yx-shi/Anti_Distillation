...
- 旧 `src/vllm_dual_decoding/run_full_pipeline.py` 已由配置驱动主入口替代并删除；`build_distill`、`train`、`rollout_eval`/`checkpoint_eval`、`eval`、`plot` 等阶段通过 `src/run_experiment.py --stage ...` 运行。
...
- 2026-05-16 已新增配置驱动主入口 `src/run_experiment.py` 与 `configs/deepscaler.yaml`、`configs/gsm8k.yaml`，统一 DeepScaleR/GSM8K 的 dataset registry、run_id、输出路径和 stage dry-run。旧 `scripts/dual_decoding/*.sh` 已迁入 `scripts/archive/dual_decoding/`。
...
- data-side smoke 质量达标后，再通过配置驱动主入口运行 full smoke：
