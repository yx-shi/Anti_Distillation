...
后续复查该 correctness-matched 结果时，rollout eval 改为每题 4 次采样，默认 `temperature=0.7`、`top_p=0.8`。新口径先计算每题 correctness mean/variance，再传播到整体 `rollout_acc_variance`；rollout accuracy 曲线需要补上 base Student 的 step 0 点，并用 mean ± std 误差线表示 eval 随机性。
